...
def get_data(data_path='./data/',label_output='./data/ml_class.txt'):
    # 读取csv数据
...
def train_tfidf_rfc(data_dir='./data/', model_dir='model_output/'):
    """
    通过词频-idf的特征输入，训练随机森林模型的先验概率。
    :param search_title:  搜索文档的类别
    :return:  vectorizer,transformer,word_bags,rfc_clf,label_dict
    """
    ...
    rfc_clf.fit(tfidf, y_train)
    # 评估测试集准确率
    from sklearn import metrics
    vector_cut_words = vectorizer.transform(X_test)
    tfidf_words = transformer.fit_transform(vector_cut_words)  # tf-idf
    result = rfc_clf.predict(tfidf_words)  # 朴素贝叶斯预测
    # tfidf_words = transformer.fit_transform(vector_cut_words)   # tf-idf
    print(metrics.classification_report(y_test, result))
    print("测试模型准确率:", metrics.accuracy_score(y_test, result))
    # 保存模型
    rfc_clf_path = model_dir + 'rfc_cls.pkl'
    with open(rfc_clf_path, 'wb') as fw:
        pickle.dump(rfc_clf, fw)
    return (vectorizer, transformer, word_bags, rfc_clf)
...
def model_predict(sentence_list):
    ...
    result = rfc_clf.predict_proba(sentence_tfidf)  # 随机森林预测
    return result
