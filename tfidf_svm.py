...
def train_tfidf_svm(data_dir='./data/', model_dir='model_output/'):
    """
    通过词频-idf的特征输入，训练随机森林模型的先验概率。
    :param search_title:  搜索文档的类别
    :return:  vectorizer,transformer,word_bags,svm_clf,label_dict
    """
    ...
    svm_clf = SVC(C=1.0, kernel="linear", probability=True)  # kernel：参数选择有rbf, linear, poly, Sigmoid, 默认的是"RBF";
    svm_clf.fit(tfidf, y_train)
    # 评估测试集准确率
    from sklearn import metrics
    vector_cut_words = vectorizer.transform(X_test)
    tfidf_words = transformer.fit_transform(vector_cut_words)  # tf-idf
    result = svm_clf.predict(tfidf_words)  # 朴素贝叶斯预测
    # tfidf_words = transformer.fit_transform(vector_cut_words)   # tf-idf
    print(metrics.classification_report(y_test, result))
    print("测试模型准确率:", metrics.accuracy_score(y_test, result))
    # 保存模型
    svm_clf_path = model_dir + 'svm_cls.pkl'
    with open(svm_clf_path, 'wb') as fw:
        pickle.dump(svm_clf, fw)
    return (vectorizer, transformer, word_bags, svm_clf)
...
def model_predict(sentence_list):
    ...
    result = svm_clf.predict_proba(sentence_tfidf)  # 支持向量机预测
    return result
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # print(model_predict(['移动应用', '南网云']))
        print(model_predict([input_text]))
